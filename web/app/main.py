...
def demo():
    if request.method == 'POST':
        ...
        sql = '''
            ...
            LIMIT 10
        '''
        data_list = clinet_bigquery(KEYFILEPATH,sql,0)
...
def flight():
    global total_amount
    # 호텔데이터불러오기(기본)
    hotel_data = clinet_bigquery(KEYFILEPATH, 
                                 '''SELECT * FROM test_db.hotelcrawl
                                    where rating != '평점없음' and star is not null
                                    LIMIT 500;''',0) 
    # 항공권 데이터 불러오기(기본)
    air_data = clinet_bigquery(KEYFILEPATH, 
                                 '''SELECT *
                                    FROM `fightproject.test_db.airplanecrawl`
                                    WHERE charge <= 50000;''',0) 
    
    # 렌터카 데이터 불러오기(기본)
    car_data = clinet_bigquery(KEYFILEPATH, 
    ...
    if request.method == 'POST':
        ...
        if 'status-select' in request.form:
            ...
            filtered_data1 = func.air_port_option_func(selected_option1, air_data, 'airport')
