...
function filterTable_hotel() {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (selectedValue !== "Choose...") {
      departureMatch =
        (selectedValue === "1" && departure === "제주") ||
        (selectedValue === "2" && departure === "서귀포");
    }
...
// 이벤트 핸들러 등록
var selectElement = document.getElementById("status-select3");
selectElement.addEventListener("change", filterTable_hotel);

var selectBox = document.getElementById("status-select4");
selectBox.addEventListener("change", filterTable_hotel);